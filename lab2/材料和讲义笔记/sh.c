...
#include <sys/stat.h>
#include <sys/wait.h>
//可以参考lab1文件夹下的xv6教程，里面对于系统调用有讲解
// Simplifed xv6 shell.
...
runcmd(struct cmd *cmd)
{
  ...
  case ' ':
    ...
    if(ecmd->argv[0] == 0)
      _exit(0);
    // Your code here ...
    //命令存在/bin也可能存在于/usr/bin下，所以我做了两手准备
    char pth[15] = "/bin/";

    if(execv(strcat(pth, ecmd->argv[0]), ecmd->argv) < 0){
      strcpy(pth, "/usr/bin/");
      printf("%s", pth);
      if(execv(strcat(pth, ecmd->argv[0]), ecmd->argv) < 0){
        printf("%s", pth);
  ...
  case '<':
    ...
    // Your code here ...
    //char *file,flags,fd
    close(rcmd->fd);//重定向之前指令fd指向显示器（也就是标准输出流），我们释放了，改写fd即重定向
    if(open(rcmd->file, rcmd->flags, S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH) < 0){
  ...
  case '|':
    ...
    if(pipe(p)<0){
      fprintf(stderr, "cannot create pipe buffer!\n");
      _exit(0);
    }
