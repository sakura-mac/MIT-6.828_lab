...
- x86 指定了栈向低地址增长：记住每个指令的对应指令
  ...
  ret								popl %eip (*)//返回，但是实际上需要返回到原eip，这代表了eip，ebp，esp都需要改变，不止这一条指令
  //*代表了这是人工造的例子
  ```
...
- GCC指定栈的使用。约定了x86中调用和被调用的情况
  ...
  - call指令：
    - 由于在每条指令执行后eip增加
    - 因此读取call指令还未执行之前，执行call指令时eip为返回后继续执行的指令
    - eip压栈
  ...
  - 在ret指令后
    ...
    - 调用者：%eax，%edx，%ecx都有可能被丢弃，不使用，
    - 调用函数：%ebp, %ebx, %esi, %edi，必须在调用期间被使用，保存。当然你得知道，如果函数需要很少，那么寄存器满足，不会被开辟栈帧，而且ebx一般指向上一个ebp
